...
func (b *GraphServiceRequestBuilder) GetDriveItemByURL(itemURL string) (*DriveItemRequestBuilder, error) {
	...
	for i := 3; i <= len(segments); i++ {
		site, err = b.Sites().GetByPath(u.Hostname(), strings.Join(segments[:i], "/")).Request().Get()
		if err == nil {
			break
		}
	}
	...
	drives, err := b.Sites().ID(*site.ID).Drives().Request().Get()
	if err != nil {
		return nil, err
	}
